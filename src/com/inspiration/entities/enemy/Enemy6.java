...
public class Enemy6 extends Entity {
	...
	public Enemy6(int x, int y, int width, int height, BufferedImage sprite) {
		super(x, y, width, height, null);

		leftEnemy = new BufferedImage[4];
		for(int i=0;i<=3;i++) {
			
		leftEnemy[i] = Game.spritesheet.getSprite(16*12+(16*i), 0, 16, 16);
		}
		rightEnemy = new BufferedImage[4];
		for(int i=0;i<=3;i++) {
			rightEnemy[i] = Game.spritesheet.getSprite(16*12+(16*i), 16, 16, 16);
		}
	...
	public boolean isColidding(int xnext, int ynext) {
		...
		for (int i = 0; i < Game.enemies6.size(); i++) {
			Enemy6 e = Game.enemies6.get(i);
			if (e == this)
				continue;
	...
	public void render(Graphics g) {
		...
		if (!isDemage) {
			...
			g.drawImage(Entity.ENEMY_FEEDBACK6, this.getX() - Camera.x, this.getY() - Camera.y, null);
		}
	}
